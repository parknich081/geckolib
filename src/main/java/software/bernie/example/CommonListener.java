...
public class CommonListener {
	...
	public static void registerEntityAttributes(EntityAttributeCreationEvent event) {
		if (!FMLEnvironment.production && !GeckoLibMod.DISABLE_IN_DEV) {
			event.put(EntityRegistry.BIKE_ENTITY.get(), Mob.createMobAttributes().add(Attributes.MAX_HEALTH, 1.0D)
					.build());
			event.put(EntityRegistry.GEO_EXAMPLE_ENTITY.get(), Mob.createMobAttributes()
